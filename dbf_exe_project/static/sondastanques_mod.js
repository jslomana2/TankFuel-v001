...
;(() => {
  async function bootstrapData(){
    try{
      ...
      // Si existe window.setData (de tu JS original), lo invocamos con datos reales.
      if (typeof window.setData === 'function'){
        window.setData(rows);
      } else {
        // Fallback mínimo: pinta tarjetas básicas en #grid
        const grid = document.getElementById('grid');
        if (grid){
          grid.innerHTML = '';
          rows.forEach(t => {
            const d = document.createElement('div');
            d.className = 'tank-card';
            d.innerHTML = '<h3>'+(t.descripcion || ('Tanque '+t.tanque_id))+'</h3>'
                        + '<p><b>Almacén:</b> '+(t.almacen_id||'')+'</p>'
                        + '<p><b>Stock:</b> '+(t.stock_l||'')+'</p>';
            d.addEventListener('click', async ()=>{
              const q = await fetch('/api/calibraciones/ultimas?tanque_id='+encodeURIComponent(t.tanque_id)+'&n=10');
              ...
              if (tbody){
                ...
                (j.rows||[]).forEach(rw => {
                  ...
                  tr.innerHTML = '<td>'+(rw.fecha||'')+' '+(rw.hora||'')+'</td>'
                               + '<td>'+(rw.litros||'')+'</td>'
                               + '<td>'+(rw.litros15||'')+'</td>'
                               + '<td>'+(rw.tempera||'')+'</td>';
      ...
      console.error('No se pudieron cargar los tanques', e);
    }
  }
