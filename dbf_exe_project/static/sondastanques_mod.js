...
(()=>{
  const API_TANQUES = "/api/tanques_norm";
  const API_CALIB   = "/api/calibraciones/ultimas";
  ...
  function pct(num, den){ if(!den) return 0; return Math.max(0, Math.min(100, (num/den)*100)); }
  function fmt(n){ return (n===null||n===""||isNaN(n)) ? "—" : new Intl.NumberFormat('es-ES').format(Number(n)); }
  function statusColor(fillPct){ if(fillPct>=90) return 'bad'; if(fillPct>=75) return 'warn'; return 'ok'; }
  ...
  function tankCard(t){  // t = row from /api/tanques_norm
    const fill = pct(t.stock_l, t.capacidad_l);
    const color = statusColor(fill);
    const id = t.tanque_id;
    const el = document.createElement('div');
    el.className = 'tank';
    el.setAttribute('role','button');
    el.setAttribute('tabindex','0');
    el.dataset.tanqueId = id;
    el.innerHTML = `
      <div class="title">${t.descripcion??''}</div>
      <div class="meta">Almacén: <b>${t.almacen_id??''}</b> · Capacidad: <b>${fmt(t.capacidad_l)} L</b></div>
      <div class="meta">Producto: <b>${(t.producto_nombre||t.producto_id)??''}</b></div>
      <div class="meta">Stock: <b>${fmt(t.stock_l)}</b> L (a 15°: <b>${fmt(t.stock15_l)}</b> L) · Temp: <b>${fmt(t.temp_ultima_c)}</b> °C</div>
      <div class="bar-wrap" aria-label="Nivel ${fill.toFixed(0)}%">
        <div class="bar" style="width:${fill.toFixed(0)}%; background: var(--${color});"></div>
      </div>
      <div class="legend" style="margin-top:8px">
        <span class="chip"><span class="dot ${color}"></span>${fill.toFixed(0)}%</span>
      </div>
    `;
    el.addEventListener('click', () => showHistorico(id, t));
    el.addEventListener('keydown', (e)=>{ if(e.key==='Enter'||e.key===' ') showHistorico(id,t); });
    return el;
  }

  async function showHistorico(tanqueId, tRow){
    try{
      histTitle.textContent = ` · tanque ${tanqueId} · ${tRow.descripcion||''}`;
      histTbody.innerHTML = '<tr><td colspan="9">Cargando…</td></tr>';
      histPanel.hidden = false;
      const url = new URL(API_CALIB, location.origin);
      // tanqueId is formatted ALM-TANQ in /api/tanques_norm
      const [alm, tan] = String(tanqueId).split('-');
      url.searchParams.set('tanque_id', tan); // backend sample worked with ?tanque_id=0001
      url.searchParams.set('n', '10');
      const data = await getJSON(url);
      const rows = (data.rows||[]);
      if(!rows.length){
        histTbody.innerHTML = '<tr><td colspan="9">Sin datos de calibración recientes.</td></tr>';
        return;
      }
      histTbody.innerHTML = rows.map(r=>`
        <tr>
          <td>${(r.FECHA||'').toString().slice(0,16)}</td>
          <td>${r.HORA||''}</td>
          <td>${r.ALMACEN||''}</td>
          <td>${r.TANQUE||''}</td>
          <td>${r.ARTICULO||''}</td>
          <td>${r.DESCRI||''}</td>
          <td>${fmt(r.LITROS)}</td>
          <td>${fmt(r.LITROS15)}</td>
          <td>${fmt(r.TEMPERA)}</td>
        </tr>
      `).join('');
    }catch(err){
      histTbody.innerHTML = `<tr><td colspan="9">Error: ${err.message}</td></tr>`;
    }
  }

  async function load(){
    try{
      grid.innerHTML = '<div class="tank">Cargando…</div>';
      const data = await getJSON(API_TANQUES);
      const rows = data.rows || [];
      // Alarmas = cuantos están sobre 90% o con stock negativo
      const alarms = rows.filter(t => pct(t.stock_l,t.capacidad_l)>=90 || Number(t.stock_l)<0).length;
      alarmCount.textContent = alarms;
      grid.innerHTML = '';
      rows.forEach(t => grid.appendChild(tankCard(t)));
    }catch(err){
      grid.innerHTML = `<div class="tank">Error cargando tanques: ${err.message}</div>`;
    }
  }
...
// --- Your original JS (kept for compatibility) ---
(function(){
