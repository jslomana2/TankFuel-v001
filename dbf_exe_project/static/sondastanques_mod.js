(function(){
  ...
  function render(){
    ...
    (a.tanques||[]).forEach(function(t){
      ...
      kv.innerHTML = "<div>Volumen</div><div><strong>"+litersLabel(t.volumen||0)+"</strong></div>"
                   ...
                   + "<div>Agua</div><div>"+(t.alturaAgua!=null? t.alturaAgua.toFixed(1)+' mm' : '-')+"</div>" + "<div>Última lectura</div><div><span class=\"lastRead\">—</span></div>";
      info.appendChild(kv);
      (async function(){ try{ var lbl = kv.querySelector('.lastRead'); var val = await _fetchLastReadingLabel(t); if(lbl && val) lbl.textContent = val; }catch(_){ } })();
  ...
  function _bestDateStrFromRow(r){
    if(!r) return null;
    var f = (r.fechahora || r.timestamp || r.fecha || r.fch || r.fec || null);
    var h = (r.hora || r.hr || null);
    if(f && h && (String(f).indexOf(":")===-1)) return String(f).trim()+" "+String(h).trim();
  ...
  async function _fetchLastReadingLabel(tank){
    try{
      var id = tank.cod || tank.codigo || tank.id || tank.nombre || "";
      if(!id) return null;
      var url = "/api/calibraciones/ultimas?tanque="+encodeURIComponent(String(id))+"&n=1";
      var r = await fetch(url);
      if(!r.ok) throw new Error("HTTP "+r.status);
      var j = await r.json();
      var lbl = j.last_ts || ( (j.rows && j.rows.length) ? _bestDateStrFromRow(j.rows[j.rows.length-1]) : null );
      return _fmtLastLabel(lbl);
    }catch(e){
      return null;
    }
  }
